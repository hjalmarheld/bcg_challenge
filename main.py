...
from model.model_pipeline import model_pipeline
...
if __name__ == "__main__":
    if config.cache:
        ...
        if not Path("cache").exists():
            Path("cache").mkdir()
        if (
            Path("cache", "train.parquet").exists()
            and Path("cache", "test.parquet").exists()
        ):
    ...
    results, live_predictions = model_pipeline(training=train, test=test)

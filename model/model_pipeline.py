...
def model_pipeline(
    ...
    # back testing
    assert window < time_points.shape[0] - 1, "window too large, > %s" % (
        time_points.shape[0] - 1
    )
    ...
    precision = (
        ...
        / (
            result.select(pl.col("true_positive")).sum()
            + result.select(pl.col("false_positive")).sum()
        )
    ).to_numpy()[0][0]
    recall = (
        result.select(pl.col("true_positive")).sum()
        / (
            result.select(pl.col("true_positive")).sum()
            + result.select(pl.col("false_negative")).sum()
        )
    ).to_numpy()[0][0]
    ...
    live_predictions = test.select(pl.col(["client_id"])).with_columns(
        pl.Series(model.predict(x_real)).alias("predicted_churn"),
